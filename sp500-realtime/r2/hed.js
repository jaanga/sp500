...
// Heads Up
	...
	function setHeadsUp( symbol ) {
		...
		sym = symbols[ symbol ];
		index = index ? index : 0;
		idx = sym.trades.length > index ? index : sym.trades.length - 1 ;
		...
		txt =
			...
			'<a href="https://www.google.com/finance?q=' + 'NASDAQ:' + sym.symbol + '" target="_blank">Google Finance: ' +
				sym.symbol + '</a><br>' +
			'chg: ' + sym.trades[ idx ][ 0 ].toFixed(1) + '%<br>' +
			'vol: ' + sym.trades[ idx ][ 1 ].toLocaleString() + '<br>' +
			'vol avg :' + sym.volumeAvg.toLocaleString() + '<br>' +
			'vol/vol avg: ' + ( 100 * sym.trades[ idx ][ 1 ] / sym.volumeAvg ).toFixed(1) + '%<br>' +
